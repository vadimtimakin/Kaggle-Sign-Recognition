...
class InputNet(tf.keras.layers.Layer):
    ...
    def call(self, xyz):
        ...
        tta_lhand, tta_rhand = self.do_hflip_hand(lhand, rhand)
        tta_spose = self.do_hflip_spose(spose)
        ...
        tta_xyz = tf.concat([
            tta_lhand, tta_rhand, tta_spose, tta_leye, tta_reye, tta_slip,
        ],axis=1)
        
        dxyz = tf.pad(xyz[:-1] - xyz[1:], [[0, 1], [0, 0], [0, 0]], mode="CONSTANT")
        tta_dxyz = tf.pad(tta_xyz[:-1] - tta_xyz[1:], [[0, 1], [0, 0], [0, 0]], mode="CONSTANT")
